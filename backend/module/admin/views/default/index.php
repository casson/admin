...
<script src="<?php echo Yii::$app->request->baseUrl; ?>/public/js/art_dialog/plugins/iframeTools.source.js" ></script>
<script src="<?php echo Yii::$app->request->baseUrl; ?>/public/js/art_dialog/_doc/highlight/highlight.pack.js"  ></script>
<script src="<?php echo Yii::$app->request->baseUrl; ?>/public/js/art_dialog/_doc/highlight/languages/javascript.js" ></script>
...
<div style="display: block;" class="scroll"><a href="javascript:;" class="per" title="<?php echo  Yii::t('admin','using the mouse wheel scrolling sidebar');?>" onclick="menuScroll(1);"></a><a href="javascript:;" class="next" title="<?php echo  Yii::t('admin','using the mouse wheel scrolling sidebar');?>" onclick="menuScroll(2);"></a></div>
 ...
 <div class="top">
	...
	<div class="admin_logo">
		<img src="<?php echo Yii::$app->request->baseUrl; ?>/public/images/admin_img/admin_logo.jpg">
	</div>
	<div style="float:left">
	<div  style="height:20px;line-height:20px;float:left;margin-left:700px;" >                                                                                          
	<?php echo Yii::t('admin','welcome').Yii::$app->session['role_name'];?> <?php echo Yii::$app->session['admin_real_name'];?> | <a href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('blog/post/index'); ?>"  onclick="$('#crumbs').html('<?php echo Yii::t('resource','my_panel');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','personal_info');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','change_password');?>');"target="main"><?php echo Yii::t('resource','change_password');?></a> | <a href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/default/logout');?>" ><?php echo Yii::t('admin','logout');?></a>
	</div>
	<div class="top_nav"   >
			<ul class="nav" >
				<?php for($i=0;$i<count($top_menus);$i++){?>
				<li ><a href="javascript:void(0)" onclick="loadLeftMenu(<?php echo $top_menus[$i]->resource_id; ?>,'<?php echo Yii::$app->urlManager->createAbsoluteUrl("".$top_menus[$i]->module."/".$top_menus[$i]->controller."/".$top_menus[$i]->action."");?>');" id='nav_li_<?php echo $top_menus[$i]->resource_id; ?>'  ><?php echo Yii::t('resource',$top_menus[$i]->name);?></a></li>
				<?php }?>
			</ul>
	</div>
	</div>
	<div class="top_member" style="display:none"  >
	<?php echo Yii::t('admin','welcome').Yii::$app->session['role_name'];?> <?php echo Yii::$app->session['admin_real_name'];?> | <a href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/system/changepassword');?>"  onclick="$('#crumbs').html('<?php echo Yii::t('resource','my_panel');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','personal_info');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','change_password');?>');"target="main"><?php echo Yii::t('resource','change_password');?></a>
	</div>
</div>
...
<div id='main_content' >
	<div class="left" style="float:left;">
		<div class="member_info" style="display:none">
				<div class="member_ico" > <a  onclick="$('#crumbs').html('<?php echo Yii::t('resource','my_panel');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','personal_info');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','account_setting');?>');" href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/system/accountsetting');?>" target="main"><img src="<?php echo Yii::$app->request->baseUrl; ?>/public/images/admin_img/nophoto.gif" width="43" height="43" ></a></div>	             <a class="system_a" onclick="$('#crumbs').html('<?php echo Yii::t('resource','my_panel');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','personal_info');?>&nbsp;>&nbsp;<?php echo Yii::t('resource','account_setting');?>');" href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/system/accountsetting');?>" target="main"><?php  echo Yii::t('resource','account_setting');?></a><a href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/default/logout');?>" class="system_logout"><?php echo Yii::t('admin','logout');?></a>
		</div>
		<div id="scroll">			
			<div id='left_main'>
	...
	<div class="right"  style="float:left">
		...
		<IFRAME id="right_main" style="margin:0px;padding:0px;" marginheight="0" name="main" src="<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/system/start/');?>" frameBorder=0 width="100%" scrolling="auto"  height="auto" allowtransparency="true"></IFRAME>
	</div>
</div>
...
function loadLeftMenu(resource_id,target_url)
{
	...
	$.get("<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/default/leftmenu/');?>", {parent_id:resource_id}, function(data){
	     $('#left_main').html(data);
		windowW();
 	}); 
...
loadLeftMenu(1,'<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/system/start');?>');
//左侧二级菜单缩放
...
function lock_screen() {
	$.get("<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/default/lockscreen/');?>");
...
function check_screenlock() {
	...
	$.get("<?php echo Yii::$app->urlManager->createAbsoluteUrl('admin/default/unlockscreen/');?>", { lock_password: lock_password},function(data){
