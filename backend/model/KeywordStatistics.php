<?php 
   ...
   class KeywordStatistics extends logCAr{
		...
		public function search($up,$end,$wtid){
			$sql ='select sum(count) as count,keywords,wtname from {{keywordstatistics}} where '.($wtid?(' wtid='.$wtid.' and'):'').' date>'.$up.' and date<='.$end.' group by wtid,keyid order by count desc '; 
